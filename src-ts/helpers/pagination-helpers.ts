// 計算偏移量: 搭配 sequelize 查詢的偏移功能, 回傳偏移量
const getOffset = (limit: number = 10, page: number = 1): number => (page - 1) * limit

// 計算頁碼器所需資訊:頁碼陣列、總共幾頁、在第幾頁、下一頁、上一頁
const getPagination = (limit: number = 10, page: number = 1, total: number = 100) => {
  ...
  const initialPages = Array.from({ length: setCounts }, (_, i) => i + 1).filter((page) => page <= totalPage)

  // 預設顯示後幾頁: 考量當頁數少於預設顯示頁數時, 將預設顯示頁數進行縮減, 防止出現零
  const finalPages = Array.from({ length: setCounts }, (_, i) => i + 1 - setCounts + totalPage).filter((page) => page > setCounts)
